...
struct my_netmap_port {
	char if_name[IFNAMSIZ];		/* contains if_xname */
	struct nm_desc 	*d;
...
netmap_fwd(struct my_netmap_port *port)
{
	...
	if (n == 0) {
		D("nothing to forward to %s", port->if_name);
		return 0;
	}
 ...
 */
extern long long handled;
int consume_pkt(char *, int, int);
...
netmap_read(struct sess *sess, void *arg)
{
	...
	/* scan all rings */
		...
		handled++;
//printf("got packet %i\n", handled);
		if(consume_pkt(buf, slot->len, port->first) ) {
			/*
 ...
 */
struct my_netmap_port *
netmap_add_port(const char *dev, struct my_netmap_port *peer)
{
	...
	strncpy(port->if_name, dev, IFNAMSIZ-1);
	port->allocator_id = port->d->req.nr_arg2;
	D("--- mem_id %d", port->allocator_id);
        port->sess = new_session(port->d->fd, netmap_read, port, WANT_READ);
